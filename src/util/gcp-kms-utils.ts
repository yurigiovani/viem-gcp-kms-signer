...
function getClientCredentials() {
  if (process.env.GOOGLE_APPLICATION_CREDENTIAL_EMAIL && process.env.GOOGLE_APPLICATION_CREDENTIAL_PRIVATE_KEY) {
    return {
      credentials: {
...
export async function requestKmsSignature(plaintext: Buffer, kmsCredentials: GcpKmsSignerCredentials) {
